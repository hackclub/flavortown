class ProjectsController < ApplicationController
  ...
  def show
    ...
    if devlog_posts.any?
      ...
      @devlog_lapse_badges = Lapse::DevlogBadgeBuilder.new(
        project: @project, devlog_posts: devlog_posts, timelapses: timelapses
      ).call
    end
  ...
  #   Displays the Lapse timelapses for a project. This has to be loaded in a Turbo frame - if it isn't, the route will redirect back to the
  #   project. This is meant to be displayed in a modal.
  def lapse_timelapses
    logger.info "Lapse timelapses requested for project #{params[:id]}"

    @project = Project.find(params[:id])
    authorize @project, :show?

    unless turbo_frame_request?
      logger.info("Lapse timelapses requested without turbo frame for project #{params[:id]}, redirecting to project page")
      redirect_to @project
      return
    end
