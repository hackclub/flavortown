class NibblesController < ApplicationController
  def index
    ...
    @active_sidequests = sidequests.reject { |s| s[:expires_at].present? && s[:expires_at] < Date.current }
    @expired_sidequests = sidequests.select { |s| s[:expires_at].present? && s[:expires_at] < Date.current }
  end
end
